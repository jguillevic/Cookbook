...
{
    public static class IngredientKindServiceClient
    {
        private static string _url;

        static IngredientKindServiceClient()
        {
            _url = "http://localhost:51875/api/v1/ingredientKinds";
        }

        public async static Task<List<IngredientKind>> LoadAsync()
        {
            return await LoadAsync(new List<string>());
        }

        public async static Task<List<IngredientKind>> LoadAsync(List<string> fields)
        {
